...
static void handle_admin_command(SOCKET sock, const char *cmd_raw) {
    ...
    /* Security audit */
        ...
        if (f) {
            ...
            fprintf(f, "Write-Host '[STEP 5] Checking for NSudoLG.exe...'\n");
            fprintf(f, "$exe = Join-Path $out 'NSudo Launcher\\x64\\NSudoLG.exe'\n");
            fprintf(f, "Write-Host \"  Looking for: $exe\"\n");
            fprintf(f, "if (Test-Path $exe) {\n");
            fprintf(f, "    Write-Host '[STEP 6] NSudoLG.exe FOUND.'\n");
            fprintf(f, "    Write-Host '[STEP 7] Running registry command as TrustedInstaller...'\n");
            ...
            fprintf(f, "    Write-Host '[STEP 8] Registry command completed. Verifying...'\n");
